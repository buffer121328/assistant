...
def main() -> None:
    ...
    if package.get("main") != "out/main/index.js":
        raise SystemExit("desktop_web_main_entry_mismatch")
    files = set(builder.get("files", []))
    if "out/**" not in files or "dist/**" in files:
        raise SystemExit("desktop_web_builder_output_mismatch")
