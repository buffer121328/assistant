...
class MemoryBaselineReport:
    """表示 处理 memory baseline report 的后端数据结构或服务对象。"""

    version: str  # version 对应的数据字段。
    token_estimator: str  # token_estimator 对应的数据字段。
    metrics: Mapping[str, Any]  # metrics 对应的数据字段。
    known_failures: tuple[Mapping[str, str], ...]  # known_failures 对应的数据字段。
...
def load_memory_baseline_fixture(path: Path) -> dict[str, Any]:
    ...
    if estimator != "unicode-unit-v1":
        raise EvaluationDataError(
            "memory baseline fixture has an unsupported token estimator"
        )
...
def evaluate_memory_baseline(path: Path) -> MemoryBaselineReport:
    ...
    cross_session = _cross_session_metrics(
        fixture["cross_session_cases"], known_failures
    )
...
def _compatibility_metrics(
    ...
    """执行 处理 compatibility metrics 的内部辅助逻辑。

    Args:
        raw_cases: raw_cases 参数。
        known_failures: known_failures 参数。
    """
...
def _isolation_metrics(
    ...
    """执行 处理 isolation metrics 的内部辅助逻辑。
...
def _record_conflicts(raw_cases: object, known_failures: list[dict[str, str]]) -> None:
    """执行 记录 conflicts 的内部辅助逻辑。
    ...
    for case in _mapping_cases(raw_cases):
        if (
            len(cast(Sequence[object], case["active_preference_ids"])) > 1
            and case.get("resolution_memory_id") is None
        ):
            _failure(
                known_failures, case, "active preferences conflict without resolution"
            )
...
def _forbidden_metrics(
    ...
    """执行 处理 forbidden metrics 的内部辅助逻辑。
...
def _truncation_metrics(
    ...
    """执行 处理 truncation metrics 的内部辅助逻辑。
    ...
    for case in cases:
        ...
        critical_ids = [
            message["id"] for message in messages if message["critical"] is True
        ]
...
def _preference_injection_metrics(raw_cases: object) -> dict[str, Any]:
    """执行 处理 preference injection metrics 的内部辅助逻辑。
    ...
    for case in cases:
        ...
        case_tokens = sum(
            _estimate_tokens(cast(str, item["text"])) for item in preferences
        )
...
def _semantic_metrics(
    ...
    """执行 处理 semantic metrics 的内部辅助逻辑。

    Args:
        raw_modes: raw_modes 参数。
        known_failures: known_failures 参数。
    """
    result: dict[str, Any] = {}
...
def _cross_session_metrics(
    ...
    """执行 处理 cross session metrics 的内部辅助逻辑。
    ...
    for case in cases:
        ...
        bucket = categories.setdefault(
            category, {"correct": 0, "total": 0, "case_ids": []}
        )
        bucket["total"] += 1
    ...
    normalized = {
        category: {
            "accuracy": _ratio(
                cast(int, values["correct"]), cast(int, values["total"])
            ),
...
def _command_gap_metrics(raw_cases: object) -> dict[str, Any]:
    """执行 处理 command gap metrics 的内部辅助逻辑。
...
def _validate_fixture_shapes(payload: Mapping[str, Any]) -> None:
    ...
    for case in _mapping_cases(payload["legacy_tasks"]):
        ...
        if (
            case.get("conversation_metadata") is not None
            or case.get("memory_metadata") is not None
        ):
            raise EvaluationDataError(
                f"memory baseline case '{case['id']}' must omit legacy metadata"
            )
    for case in _mapping_cases(payload["isolation_cases"]):
        owned = _required_mapping(
            case.get("owned_memory_ids"), cast(str, case["id"]), "owned_memory_ids"
        )
        observed = _required_mapping(
            case.get("observed_memory_ids"),
            cast(str, case["id"]),
            "observed_memory_ids",
        )
        if set(owned) != set(observed):
            raise EvaluationDataError(
                f"memory baseline case '{case['id']}' has mismatched users"
            )
        for user_id in owned:
            _string_list(
                owned[user_id], cast(str, case["id"]), f"owned_memory_ids.{user_id}"
            )
            _string_list(
                observed[user_id],
                cast(str, case["id"]),
                f"observed_memory_ids.{user_id}",
            )
    for case in _mapping_cases(payload["conflict_cases"]):
        _string_list(
            case.get("active_preference_ids"),
            cast(str, case["id"]),
        ...
        if resolution is not None and not isinstance(resolution, str):
            raise EvaluationDataError(
                f"memory baseline case '{case['id']}' has invalid resolution"
            )
    for case in _mapping_cases(payload["forbidden_samples"]):
        ...
        for field in ("expected_action", "observed_action"):
            if _required_string(case.get(field), cast(str, case["id"]), field) not in {
                "reject",
                "accept",
            }:
                raise EvaluationDataError(
                    f"memory baseline case '{case['id']}' has invalid {field}"
                )
    for case in _mapping_cases(payload["long_conversations"]):
        _positive_int(case.get("window_size"), cast(str, case["id"]), "window_size")
        _required_bool(
            case.get("answer_affected"), cast(str, case["id"]), "answer_affected"
        )
        messages = _required_list(
            case.get("messages"), cast(str, case["id"]), "messages"
        )
        _validate_case_ids(messages, f"{case['id']}.messages")
        for message in _mapping_cases(messages):
            _required_bool(
                message.get("critical"), cast(str, message["id"]), "critical"
            )
    for case in _mapping_cases(payload["preference_injection_cases"]):
        preferences = _required_list(
            case.get("preferences"), cast(str, case["id"]), "preferences"
        )
    ...
    if {mode.get("mode") for mode in modes} != {"enabled", "disabled"}:
        raise EvaluationDataError(
            "memory baseline fixture requires enabled and disabled semantic modes"
        )
    for mode in modes:
        cases = (
            _required_list(mode.get("cases"), cast(str, mode["id"]), "cases")
            if "id" in mode
            else _required_list(mode.get("cases"), cast(str, mode["mode"]), "cases")
        )
        for case in _mapping_cases(cases):
            relevant = _non_negative_int(
                case.get("relevant_total"), cast(str, case["id"]), "relevant_total"
            )
            retrieved = _non_negative_int(
                case.get("retrieved_relevant"),
                cast(str, case["id"]),
                "retrieved_relevant",
            )
            if retrieved > relevant:
                raise EvaluationDataError(
                    f"memory baseline case '{case['id']}' retrieves more relevant items than exist"
                )
            _non_negative_int(
                case.get("latency_ms"), cast(str, case["id"]), "latency_ms"
            )
            _required_bool(case.get("failed"), cast(str, case["id"]), "failed")
    for case in _mapping_cases(payload["cross_session_cases"]):
        if _required_string(
            case.get("category"), cast(str, case["id"]), "category"
        ) not in {"preference", "knowledge_update", "temporal", "abstention"}:
            raise EvaluationDataError(
                f"memory baseline case '{case['id']}' has invalid category"
            )
        _required_bool(case.get("correct"), cast(str, case["id"]), "correct")
    for case in _mapping_cases(payload["stale_memory_cases"]):
        _required_bool(
            case.get("used_stale_memory"), cast(str, case["id"]), "used_stale_memory"
        )
    for case in _mapping_cases(payload["memory_command_gaps"]):
        if _required_string(
            case.get("command"), cast(str, case["id"]), "command"
        ) not in {"write", "list", "delete"}:
            raise EvaluationDataError(
                f"memory baseline case '{case['id']}' has invalid command"
            )
        if _required_string(
            case.get("severity"), cast(str, case["id"]), "severity"
        ) not in {"low", "medium", "high"}:
            raise EvaluationDataError(
                f"memory baseline case '{case['id']}' has invalid severity"
            )
...
def _load_mapping(path: Path) -> Mapping[str, Any]:
    ...
    try:
        ...
        raise EvaluationDataError(
            f"unable to load memory baseline JSON '{path.name}'"
        ) from exc
    if not isinstance(payload, Mapping):
        raise EvaluationDataError(
            f"memory baseline JSON '{path.name}' must be an object"
        )
    return cast(Mapping[str, Any], payload)


def _validate_safe_values(
    value: object, *, case_id: str = "fixture", field: str = "root"
) -> None:
    """执行 校验 safe values 的内部辅助逻辑。
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in value.items():
            _validate_safe_values(
                item, case_id=cast(str, next_case_id), field=f"{field}.{key}"
            )
        return
...
def _require_exact_terms(
    ...
    """执行 处理 require exact terms 的内部辅助逻辑。

    Args:
        taxonomy: taxonomy 参数。
        field: field 参数。
...
def _validate_case_ids(cases: Sequence[object], section: str) -> None:
    ...
    for raw_case in cases:
        if not isinstance(raw_case, Mapping):
            raise EvaluationDataError(
                f"memory baseline section '{section}' has an invalid case"
            )
        case_id = _required_string(raw_case.get("id"), section, "id")
        if not CASE_ID_PATTERN.fullmatch(case_id):
            raise EvaluationDataError(
                f"memory baseline section '{section}' has an invalid case id"
            )
        ids.append(case_id)
    if len(ids) != len(set(ids)):
        raise EvaluationDataError(
            f"memory baseline section '{section}' has duplicate case ids"
        )


def _mapping_cases(value: object) -> list[Mapping[str, Any]]:
    """执行 处理 mapping cases 的内部辅助逻辑。
    ...
    if not isinstance(value, Sequence) or isinstance(value, str | bytes | bytearray):
        raise EvaluationDataError(
            "memory baseline fixture has an invalid case collection"
        )
...
def _required_mapping(value: object, case_id: str, field: str) -> Mapping[str, Any]:
    """执行 处理 required mapping 的内部辅助逻辑。
    ...
    if not isinstance(value, Mapping):
        raise EvaluationDataError(
            f"memory baseline case '{case_id}' has invalid {field}"
        )
    return cast(Mapping[str, Any], value)


def _required_list(value: object, case_id: str, field: str) -> list[Any]:
    """执行 处理 required list 的内部辅助逻辑。
...
def _non_negative_int(value: object, case_id: str, field: str) -> int:
    """执行 处理 non negative int 的内部辅助逻辑。
