...
REASON_SCREEN_DESKTOP_GROUP_DENIED = "screen_desktop_group_denied"


NormalizedLangBotMessage = IncomingMessage
...
class LangBotResultClient:
    ...
    async def send_image(
        ...
        *,
        ...
        """Send an image payload through LangBot when the adapter supports it.

        Args:
            adapter: 用于执行当前操作的 adapter 参数。
            conversation_id: 目标会话 ID。
            conversation_type: 用于执行当前操作的 conversation type 参数。
            image_reference: 用于执行当前操作的 image reference 参数。
            mime_type: 用于执行当前操作的 mime type 参数。
            text: 需要处理的文本内容。
        ...
        try:
            ...
            raise RuntimeError("LangBot image push timed out") from exc
        except httpx.TransportError as exc:
            raise RuntimeError(str(exc)) from exc
        ...
        try:
            ...
            raise RuntimeError("LangBot image push returned invalid JSON") from exc
        if not isinstance(data, dict):
            raise RuntimeError("LangBot image push returned invalid response shape")
        return data
...
async def handle_langbot_webhook(
    *,
    ...
    if task_type == "screen":
        try:
            screen_command = parse_screen_command(
                message.text,
                default_target=settings.desktop_capture_default_target,
            )
        except ValueError:
            return await record_no_task_ack(
                session=session,
        ...
        if (
            screen_command.target == "desktop"
            and message.conversation_type != "person"
            and settings.desktop_capture_require_confirmation_for_desktop
        ):
            return await record_no_task_ack(
                session=session,
                repository=repository,
                message=message,
                reason=REASON_SCREEN_DESKTOP_GROUP_DENIED,
                intent_outcome="screen",
            )
