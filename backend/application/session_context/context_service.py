...
from application.session_context.context_blocks import (
    BLOCK_ORDER,
    BLOCK_TYPES,
...
from application.session_context.text import safe_text
    ...
    async def ensure_summary_current(
        ...
        *,
        ...
        Args:
            conversation_id: 目标会话 ID。
            user_id: 目标用户 ID。
            summarizer: 用于执行当前操作的 summarizer 参数。
            policy: 用于执行当前操作的 policy 参数。
            summary_version: 用于执行当前操作的 summary version 参数。
            model_version: 用于执行当前操作的 model version 参数。
            exclude_task_id: 用于执行当前操作的 exclude task id 参数。
        """
    ...
    async def upsert_block(
        ...
        *,
        ...
        Args:
            ...
            read_only: 用于执行当前操作的 read only 参数。
            update_policy: 用于执行当前操作的 update policy 参数。
            allow_read_only_update: 用于执行当前操作的 allow read only update 参数。
        """
        validate_block_identity(
    ...
    async def list_blocks(
        ...
        *,
        ...
        Args:
            user_id: 目标用户 ID。
            conversation_id: 目标会话 ID。
            project_id: 用于执行当前操作的 project id 参数。
        """
        now = utc_now()
        scopes = [
