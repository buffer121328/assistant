...
from application.task_execution.commands import _safe_json, _safe_summary
from application.task_execution.lifecycle import TaskNotFoundError
from domain.policies.task_status import DISPATCHABLE_TASK_STATUSES
from application.desktop_capture.service import SCREEN_TOOL_NAME, screen_artifact_from_tool_log
from domain.models import ProcessedMessage, Task, TaskStatus, ToolLog
...
class ResultDispatcher:
    ...
    async def dispatch_task(self, task_id: str) -> DispatchResult:
        ...
        try:
            if task.task_type == "screen" and task.status == TaskStatus.SUCCESS.value:
                response_payload = await self._send_screen_result(
                    task=task,
                    target=target,
                ...
                response_payload = {"mode": "text", "response": response}
    ...
    async def _send_screen_result(
        ...
        *,
        ...
        """优先发送截图产物，失败或不支持时退回发送文本摘要。

        Args:
            task: 需要处理的任务对象。
            target: 用于执行当前操作的 target 参数。
            outbound_text: 用于执行当前操作的 outbound text 参数。
        """
        artifact = await self._latest_screen_artifact(task.id)
        if artifact is None:
            response = await self._send_message(
                task=task,
                target=target,
        ...
        image_reference = artifact.get("url") or artifact["reference"]
        send_image = getattr(self.langbot_client, "send_image", None)
        if callable(send_image):
            try:
                response = await send_image(
                    ...
                    text=f"截图已生成。任务ID: {task.id}",
                    idempotency_key=f"{task.id}:image",
                )
                return {"mode": "image", "artifact": artifact, "response": response}
            except Exception as exc:
                fallback_text = self._build_screen_fallback_message(
                    task=task,
                    artifact=artifact,
        ...
        return {
            "mode": "text_fallback",
            "fallback_reason": "image_send_unavailable",
            "artifact": artifact,
            "response": response,
        }

    def _build_screen_fallback_message(
        self,
        *,
        task: Task,
        ...
        """根据截图产物和发送失败原因构造文本回退消息。

        Args:
            task: 需要处理的任务对象。
            artifact: 用于执行当前操作的 artifact 参数。
            reason: 用于执行当前操作的 reason 参数。
        """
        reason_text = (
            "当前 LangBot 适配器未成功发送图片"
            if reason == "image_send_failed"
            else "当前 LangBot 客户端未配置图片发送能力"
        )
        lines = [
            f"截图已生成，但{reason_text}。",
            f"任务ID: {task.id}",
            f"截图文件: {artifact.get('filename', 'screenshot.png')}",
        ]
        url = artifact.get("url")
        if isinstance(url, str) and url:
            lines.append(f"链接: {url}")
        else:
            lines.append("请在本地桌面端查看 artifact。")
        return "\n".join(lines)

    async def _latest_screen_artifact(self, task_id: str) -> dict[str, Any] | None:
        """读取任务最近一次成功截图工具调用并解析其产物引用。
        ...
        result = await self.session.scalars(
            ...
            .where(
                ...
                ToolLog.tool_name == SCREEN_TOOL_NAME,
                ToolLog.status == "succeeded",
            )
            .order_by(ToolLog.created_at.desc(), ToolLog.id.desc())
            .limit(1)
        )
        log = result.first()
        if log is None:
            return None
        return screen_artifact_from_tool_log(log.output_text)
