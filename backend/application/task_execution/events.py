...
from domain.models import AgentRun, Task, TaskEvent
from runtime.contracts import normalize_agent_event
...
class TaskEventRepository:
    ...
    async def append(
        ...
        *,
        ...
        tenant_id = task.tenant_id if task is not None else "local"
        if run_id is None:
        ...
        try:
            ...
            normalized_payload = _safe_json_value(payload)
        safe = json.dumps(
            normalized_payload,
            ensure_ascii=False,
            default=str,
...
def event_record(item: TaskEvent) -> dict[str, object]:
    ...
    return {
        ...
        "normalized_type": item.normalized_event_type or item.event_type,
        "run_id": item.run_id,
        "payload": payload,
