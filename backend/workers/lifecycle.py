...
from runtime.contracts import select_execution_mode
    ...
    for _ in range(3):
        ...
        agent_run = AgentRun(
            ...
            execution_mode=select_execution_mode(
                allows_subagents=task.task_type == "plan",
                parallel_steps=2 if task.workflow_key else 0,
            ).value,
        )
...
async def finish_agent_run(
    ...
    *,
    ...
    Args:
        session: 当前数据库异步会话。
        agent_run: 用于执行当前操作的 agent run 参数。
        task: 需要处理的任务对象。
