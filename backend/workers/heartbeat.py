...
from integrations.notification_delivery import ReminderService, deliver_langbot_due
...
def _monitoring_dispatch(
    ...
    """把可选的任务分发回调适配为监控器需要的异步函数。

    Args:
        dispatch_task: 用于执行当前操作的 dispatch task 参数。
    """
    if dispatch_task is None:
        return None

    async def dispatch(task_id: str) -> None:
        """转发监控器发现的任务 ID 到实际任务分发回调。
