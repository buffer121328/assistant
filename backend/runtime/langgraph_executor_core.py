...
if TYPE_CHECKING:
    ...
    from agent.prompting.model_contract import AgentModelProtocol
...
class LangGraphExecutor(
    ...
    def __init__(
        ...
        *,
        ...
        self.content_guard = content_guard or NoopContentGuard()
    ...
    async def execute(self, *, run_input: AgentRunInput) -> AgentRunResult:
        ...
        callback_provider = getattr(self.observability, "langgraph_callbacks", lambda: ())
        langgraph_callbacks = tuple(callback_provider())
        if langgraph_callbacks:
            config["callbacks"] = list(langgraph_callbacks)
            config["run_name"] = "agent.langgraph"
        ...
        if self.content_guard.enabled:
            source_urls = tuple(
                str(item.get("url"))
                for item in final_state.get("sources", [])
                if isinstance(item, dict) and isinstance(item.get("url"), str)
            )
            guard_decision = await self.content_guard.inspect_final_result(
            ...
            if guard_decision.blocked:
                raise ContentGuardBlockedError(guard_decision)
            if guard_decision.text is None:
                raise RuntimeError("Content governance returned no final result")
            result_text = guard_decision.text
        return AgentRunResult(
    ...
    async def _record_content_decision(
        self,
        *,
        scope: str,
        decision: ContentGuardDecision,
        tool_name: str | None,
    ) -> None:
        """Publish bounded content-governance metadata without original content.
        ...
        """
        if self.event_sink is None or not self.content_guard.enabled:
            return
        ...
        if decision.content_fingerprint is not None:
            payload["content_fingerprint"] = decision.content_fingerprint
        try:
            await self.event_sink("governance.content", payload)
        except Exception:
            # Event-publisher failures must not permit or block content by themselves.
            return
