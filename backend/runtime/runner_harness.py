...
class AgentHarness:
    ...
    async def _publish_event(self, event_type: str, payload: dict[str, object]) -> None:
        ...
        Args:
            ...
            payload: payload 参数。
        """
        current_task = await self.session.get(Task, self._current_task_id)
        safe_payload = safe_event_payload(payload)
        await publish_legacy_event(self.event_sink, event_type, safe_payload)
        if self.lifecycle_registry is None or current_task is None:
            return
        try:
            event = build_lifecycle_event(
                event_type=event_type,
                actor_user_id=current_task.user_id,
                tenant_id=current_task.tenant_id,
                organization_id=current_task.organization_id,
        ...
        *,
        ...
        if self.task_lifecycle is None:
            ...
            task_lifecycle = build_task_lifecycle(
                self.session,
                success_hook=memory_candidate_as_success_hook(
                    self.memory_candidate_hook
                ),
            )
        else:
