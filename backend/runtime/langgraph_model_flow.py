...
class ModelFlowMixin:
    ...
    async def _model(
        ...
        async def decide() -> _ExecutionState:
            ...
            request = build_agent_model_request(
                ...
                stream_answer=not self.content_guard.enabled,
                protect_final_answer_logs=self.content_guard.enabled,
            )
        ...
        async def delegate() -> _ExecutionState:
            ...
            requests = tuple(
                SubAgentRequest(
                    ...
                    conversation_id=run_input.context.conversation_id,
                )
    ...
    async def _plan(
        ...
        async def create_plan() -> _ExecutionState:
            """创建 plan。"""
            from agent.prompting.model_contract import build_work_plan_request
