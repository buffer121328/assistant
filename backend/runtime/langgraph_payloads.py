...
if TYPE_CHECKING:
    from agent.prompting.model_contract import AgentDecision, WorkPlan
    from agent.ports import HumanApprovalRequest
...
def _approval_requests_from_interrupts(
    ...
    """
    from agent.ports import ApprovalTypeName, HumanApprovalRequest
        ...
        if (
            ...
            request = HumanApprovalRequest(
                ...
                request_fingerprint=(
                    ...
                    if isinstance(value.get("request_fingerprint"), str)
                    else None
                ),
            )
...
def _work_plan_from_state(state: _ExecutionState) -> WorkPlan | None:
    ...
    Args:
        state: state 参数。
    """
    from agent.prompting.model_contract import WorkPlan, WorkPlanStep
