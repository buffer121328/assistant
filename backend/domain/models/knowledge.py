...
class KnowledgeDocument(TimestampMixin, Base):
    ...
    tenant_id: Mapped[str] = mapped_column(ForeignKey("tenants.id"), default=LOCAL_TENANT_ID, server_default=LOCAL_TENANT_ID, nullable=False)
    organization_id: Mapped[str | None] = mapped_column(ForeignKey("organizations.id"), nullable=True)
    owner_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
    visibility: Mapped[str] = mapped_column(String(32), default=ResourceVisibility.PRIVATE.value, server_default=ResourceVisibility.PRIVATE.value, nullable=False)
