...
from domain.services import ForbiddenMemoryContentError, MemoryNotFoundError, MemoryService
...
class AgentMemoryToolService:
    ...
    async def remember(self, invocation: ToolInvocation) -> dict[str, object]:
        ...
        source_trust = str(invocation.arguments.get("source_trust") or ("trusted_user" if explicit else "untrusted_external"))
        if source_trust != "trusted_user":
            explicit = False
        safety = classify_memory_sensitivity(content)
        if safety.sensitivity == "forbidden":
            await self._audit(invocation, "memory.remember", "failed", {"reason": safety.reason_code}, None)
        ...
        memory = await service.create_memory(
            ...
            reason_code="explicit_user_request" if explicit else "agent_inferred_candidate",
            source_task_id=invocation.task_id,
            confirmed_by_user=explicit,
        )
        await self._audit(invocation, "memory.remember", "succeeded", {"memory_id": memory.id, "status": memory.status, "sensitivity": memory.sensitivity}, None)
        return {
            "memory_id": memory.id,
    ...
    async def recall(self, invocation: ToolInvocation) -> dict[str, object]:
        ...
        token_budget = _positive_int(invocation.arguments.get("token_budget"), self.token_budget)
        ...
        payload: dict[str, object] = {
            ...
            "trace": {
                ...
                "retrieval_mode": "fallback" if "fallback" in result.mode else result.mode,
        ...
        await self._audit(invocation, "memory.recall", "succeeded", {"count": len(result.items), "trace_id": result.trace_id, "query_type": query_type, "mode": "fallback" if "fallback" in result.mode else result.mode}, None)
        return payload

    async def forget(self, invocation: ToolInvocation) -> dict[str, object]:
        memory_id = str(invocation.arguments.get("memory_id") or "")
        reason = str(invocation.arguments.get("reason") or "user_requested_forget")[:500]
        service = MemoryService(self.session, semantic_memory=self.semantic_memory)
        try:
            memory = await service.get_memory(user_id=invocation.user_id, memory_id=memory_id)
        except MemoryNotFoundError:
            await self._audit(invocation, "memory.forget", "failed", {"memory_id": memory_id, "reason": "not_owned_or_missing"}, None)
            raise
        memory.status = "archived"
        memory.is_active = False
        memory.archived_at = memory.archived_at or memory.updated_at
        await service.add_feedback(user_id=invocation.user_id, memory_id=memory.id, feedback_type="forgotten", task_id=invocation.task_id)
        await self.session.flush()
        await self._audit(invocation, "memory.forget", "succeeded", {"memory_id": memory.id, "status": memory.status, "reason": reason}, None)
        return {"memory_id": memory.id, "status": memory.status, "archived": True}

    async def _audit(self, invocation: ToolInvocation, tool_name: str, status: str, output: object | None, error: str | None) -> None:
        self.session.add(
            ToolLog(
                task_id=invocation.task_id,
                tool_name=tool_name,
                status=status,
                input_text=sanitize_text({"tool": tool_name, "task_id": invocation.task_id, "user_id": invocation.user_id, "arguments": _safe_arguments(invocation.arguments)}),
...
def build_memory_tool_descriptors(*, enabled: bool = True) -> tuple[ToolDescriptor, ...]:
    return tuple(
        ToolDescriptor(
            name=name,
...
_MEMORY_TOOL_DEFS: tuple[tuple[str, str, str, dict[str, Any]], ...] = (
    ("memory.remember", "Store an explicit or candidate user memory through MemoryService", "L2", {"type": "object", "properties": {"content": {"type": "string"}, "memory_type": {"type": "string"}, "source": {"type": "string"}, "source_trust": {"type": "string"}, "explicit": {"type": "boolean"}}, "required": ["content"], "additionalProperties": False}),
    ("memory.recall", "Recall bounded owner-scoped memories with trace summary", "L1", {"type": "object", "properties": {"query": {"type": "string"}, "max_items": {"type": "integer"}, "token_budget": {"type": "integer"}, "scope_kind": {"type": "string"}, "scope_id": {"type": "string"}}, "required": ["query"], "additionalProperties": False}),
    ("memory.forget", "Archive an owned memory by default", "L2", {"type": "object", "properties": {"memory_id": {"type": "string"}, "reason": {"type": "string"}}, "required": ["memory_id"], "additionalProperties": False}),
)
