...
class AgentScheduleService:
    ...
    async def create(
        ...
        *,
        ...
        await self._tool_log(
            ...
            status="succeeded",
            output={"schedule_id": schedule.id, "mode": schedule.mode, "next_run_at": schedule.next_run_at},
        )
    ...
    async def toggle(self, *, user_id: str, schedule_id: str, enabled: bool) -> dict[str, object]:
        schedule = await self._owned_schedule(user_id=user_id, schedule_id=schedule_id)
        schedule.enabled = enabled
        await self._tool_log(task_id=None, name="schedule.toggle", status="succeeded", output={"schedule_id": schedule.id, "enabled": enabled})
        await self.session.commit()
    ...
    async def delete(self, *, user_id: str, schedule_id: str) -> dict[str, object]:
        ...
        await self._tool_log(task_id=None, name="schedule.delete", status="succeeded", output={"schedule_id": schedule.id})
        await self.session.commit()
        return {"schedule_id": schedule.id, "deleted": True}

    async def run_now(self, *, user_id: str, schedule_id: str) -> dict[str, object]:
        schedule = await self._owned_schedule(user_id=user_id, schedule_id=schedule_id)
        run = await self._materialize(schedule, scheduled_for=utc_now(), evaluated_at=utc_now(), force=True)
        await self.session.commit()
        return {"schedule_id": schedule.id, "run_id": run.id, "task_id": run.task_id, "status": run.status}

    async def history(self, *, user_id: str, schedule_id: str, limit: int = MAX_HISTORY) -> list[dict[str, object]]:
        schedule = await self._owned_schedule(user_id=user_id, schedule_id=schedule_id)
        result = await self.session.scalars(
    ...
    async def materialize_due(self, *, now: datetime | None = None) -> list[AgentScheduleRun]:
        ...
        for schedule in result:
            ...
            runs.append(await self._materialize(schedule, scheduled_for=schedule.next_run_at, evaluated_at=current))
        await self.session.commit()
        return runs
    ...
    async def _materialize(
        ...
        *,
        ...
        await TaskEventRepository(self.session).append(
            ...
            user_id=schedule.user_id,
            event_type="queued",
            payload={"source": "schedule", "schedule_id": schedule.id, "scheduled_for": scheduled_for.isoformat()},
        )
        schedule.last_run_at = scheduled_for
        schedule.next_run_at = self._next_after_run(schedule, scheduled_for, evaluated_at=evaluated_at or scheduled_for)
        await self._tool_log(task_id=task.id, name="schedule.materialize", status="succeeded", output={"schedule_id": schedule.id, "run_id": run.id})
        return run

    def _next_after_run(self, schedule: AgentSchedule, scheduled_for: datetime, *, evaluated_at: datetime) -> datetime | None:
        payload = json.loads(schedule.payload_json)
        if schedule.mode == "at":
            schedule.enabled = False
            return None
        ...
        if schedule.mode == "cron":
            return _next_cron_time(str(payload.get("cron_expr") or "* * * * *"), scheduled_for + timedelta(minutes=1), _timezone(schedule.timezone))
        return None
    ...
    async def _tool_log(self, *, task_id: str | None, name: str, status: str, output: dict[str, object]) -> None:
        self.session.add(ToolLog(task_id=task_id, tool_name=name, status=status, output_text=json.dumps(output, ensure_ascii=False, default=str)))
        await self.session.flush()

    @staticmethod
    def _schedule_dict(item: AgentSchedule) -> dict[str, object]:
        return {"schedule_id": item.id, "mode": item.mode, "enabled": item.enabled, "next_run_at": item.next_run_at.isoformat() if item.next_run_at else None, "last_run_at": item.last_run_at.isoformat() if item.last_run_at else None, "timezone": item.timezone, "catch_up_policy": item.catch_up_policy}

    @staticmethod
    def _run_dict(item: AgentScheduleRun) -> dict[str, object]:
        return {"run_id": item.id, "schedule_id": item.schedule_id, "task_id": item.task_id, "scheduled_for": item.scheduled_for.isoformat(), "status": item.status}


def build_schedule_tool_descriptors(*, enabled: bool = True) -> tuple[ToolDescriptor, ...]:
    return tuple(
        ToolDescriptor(name=name, description=description, input_schema=schema, source_id="builtin", source_kind="builtin", version=SCHEDULE_TOOL_VERSION, enabled=enabled, risk_level=cast(ToolRiskLevel, risk), requires_approval=risk != "L1", tags=("schedule", "automation", "v10"), parallel_safe=risk == "L1")
...
def build_schedule_tool_specs(service: AgentScheduleService) -> tuple[ToolSpec, ...]:
    ...
    async def toggle(invocation: ToolInvocation) -> Any:
        return await service.toggle(user_id=invocation.user_id, schedule_id=str(invocation.arguments.get("schedule_id") or ""), enabled=bool(invocation.arguments.get("enabled")))

    async def run_now(invocation: ToolInvocation) -> Any:
        return await service.run_now(user_id=invocation.user_id, schedule_id=str(invocation.arguments.get("schedule_id") or ""))

    async def delete(invocation: ToolInvocation) -> Any:
        return await service.delete(user_id=invocation.user_id, schedule_id=str(invocation.arguments.get("schedule_id") or ""))

    async def history(invocation: ToolInvocation) -> Any:
        return await service.history(user_id=invocation.user_id, schedule_id=str(invocation.arguments.get("schedule_id") or ""), limit=int(invocation.arguments.get("limit") or MAX_HISTORY))

    handlers = {"schedule.create": create, "schedule.list": list_schedules, "schedule.toggle": toggle, "schedule.run_now": run_now, "schedule.delete": delete, "schedule.history": history}
    return tuple(ToolSpec(name=name, description=description, risk_level=cast(ToolRiskLevel, risk), handler=handlers[name], input_schema=schema, version=SCHEDULE_TOOL_VERSION, source_id="builtin") for name, description, risk, schema in _SCHEDULE_TOOL_DEFS)  # type: ignore[arg-type]


def _safe_payload(payload: dict[str, object]) -> dict[str, object]:
    allowed = {"task_type", "input_text", "workflow_key", "model_class"}
    safe = {key: sanitize_text(value) if isinstance(value, str) else value for key, value in payload.items() if key in allowed}
...
_PAYLOAD = {"type": "object", "properties": {"task_type": {"type": "string"}, "input_text": {"type": "string"}, "workflow_key": {"type": "string"}, "model_class": {"type": "string"}}, "required": ["input_text"], "additionalProperties": False}
_SCHEDULE_TOOL_DEFS: tuple[tuple[str, str, str, dict[str, Any]], ...] = (
    ("schedule.create", "Create an at/every/cron schedule", "L2", {"type": "object", "properties": {"mode": {"type": "string", "enum": ["at", "every", "cron"]}, "payload": _PAYLOAD, "conversation_id": {"type": "string"}, "run_at": {"type": "string"}, "every_seconds": {"type": "integer"}, "cron_expr": {"type": "string"}, "timezone": {"type": "string"}, "catch_up_policy": {"type": "string", "enum": ["skip", "catch_up"]}}, "required": ["mode", "payload"], "additionalProperties": False}),
    ("schedule.list", "List owned schedules", "L1", {"type": "object", "properties": {}, "additionalProperties": False}),
    ("schedule.toggle", "Enable or pause an owned schedule", "L2", {"type": "object", "properties": {"schedule_id": _ID, "enabled": {"type": "boolean"}}, "required": ["schedule_id", "enabled"], "additionalProperties": False}),
    ("schedule.run_now", "Materialize an owned schedule immediately", "L2", {"type": "object", "properties": {"schedule_id": _ID}, "required": ["schedule_id"], "additionalProperties": False}),
    ("schedule.delete", "Delete an owned schedule", "L2", {"type": "object", "properties": {"schedule_id": _ID}, "required": ["schedule_id"], "additionalProperties": False}),
    ("schedule.history", "Read bounded owned schedule run history", "L1", {"type": "object", "properties": {"schedule_id": _ID, "limit": {"type": "integer"}}, "required": ["schedule_id"], "additionalProperties": False}),
)
