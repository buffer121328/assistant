...
class SkillCandidate:
    ...
    trust_level: str  # trust_level 对应的数据字段。
    capability_match: float  # capability_match 对应的数据字段。
    dependency_risk: str = "low"  # dependency_risk 对应的数据字段。
    permission_risk: str = "low"  # permission_risk 对应的数据字段。
    maintenance: float = 0.8  # maintenance 对应的数据字段。
    testability: float = 0.8  # testability 对应的数据字段。
    overlap: float = 0.0  # overlap 对应的数据字段。
    requires_approval: bool = True  # requires_approval 对应的数据字段。
    score: float = 0.0  # score 对应的数据字段。
...
class SkillAcquisitionDecision:
    ...
    decision: SkillAcquisitionDecisionType  # decision 对应的数据字段。
    candidates: tuple[SkillCandidate, ...] = ()  # candidates 对应的数据字段。
    local_skill: SkillInventoryItem | None = None  # local_skill 对应的数据字段。
    risk_level: str = "L1"  # risk_level 对应的数据字段。
    requires_approval: bool = False  # requires_approval 对应的数据字段。
    reason: str = ""  # reason 对应的数据字段。
    composed_tools: tuple[str, ...] = ()  # composed_tools 对应的数据字段。
...
class SkillPackageMetadata:
    ...
    candidate: SkillCandidate  # candidate 对应的数据字段。
    package_size: int  # package_size 对应的数据字段。
    checksum: str | None = None  # checksum 对应的数据字段。
...
class SkillAcquisitionService:
    ...
    lifecycle: SkillLifecycleService  # lifecycle 对应的数据字段。
    marketplace_providers: Sequence[SkillMarketplaceProvider] = field(
        default_factory=tuple
    )  # marketplace_providers 对应的数据字段。
    installed_revision: int = 0  # installed_revision 对应的数据字段。
