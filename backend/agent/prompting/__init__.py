...
from domain.models import Approval, ApprovalStatus, ApprovalType, EvolutionChange, EvolutionVersion, Task, TaskStatus, utc_now
from model_gateway import sanitize_text

PromptModuleName = Literal["SYSTEM", "MEMORY_GUIDE", "TOOL_POLICY", "RESPONSE_STYLE", "AGENT_CONFIG"]
...
_POLICY_DOWNGRADE = re.compile(r"(?i)(disable|关闭|绕过|bypass).{0,30}(approval|审批|toolregistry|tool registry|risk|风险|permission|权限)")
_SECRET_LIKE = re.compile(r"(?i)(api[_-]?key|access[_-]?token|refresh[_-]?token|secret|password|passwd)\s*[:=]\s*\S+|-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----|\bsk-[A-Za-z0-9_-]{8,}")
...
class PromptStore:
    def __init__(self, *, defaults_root: Path, managed_root: Path, max_module_bytes: int = 64_000) -> None:
    ...
    def build(self, runtime_context: dict[str, Any] | None = None) -> PromptBuildResult:
        ...
        parts = [f"<!-- module:{module.name} source:{module.source} fingerprint:{module.fingerprint[:12]} -->\n{module.content.strip()}" for module in modules]
        ...
        return PromptBuildResult(
            ...
            metadata={
                ...
                "modules": [
                    {"name": module.name, "source": module.source, "fingerprint": module.fingerprint, "version": module.version}
    ...
    def load_module(self, name: str) -> PromptModule:
        ...
        if _safe_existing_file(managed, self.managed_root, self.max_module_bytes):
            ...
            return PromptModule(module_name, filename, content, "managed", _checksum(content), self._latest_version(filename))
        default = self.defaults_root / filename
        if not _safe_existing_file(default, self.defaults_root, self.max_module_bytes):
            raise PromptValidationError(f"Default prompt module is unavailable: {module_name}")
        content = default.read_text(encoding="utf-8")
        return PromptModule(module_name, filename, content, "default", _checksum(content), None)

    def inspect(self) -> dict[str, object]:
        modules = [self.load_module(name) for name in MODULE_FILES]
        return {"modules": [{"name": item.name, "source": item.source, "fingerprint": item.fingerprint, "summary": item.content.strip()[:160], "version": item.version} for item in modules]}

    async def propose_change(self, *, session: AsyncSession, task_id: str, user_id: str, module_name: str, content: str, evidence: str) -> EvolutionChange:
        ...
        change = EvolutionChange(
            ...
            validation_result=json.dumps({"module": module, "path_safe": True, "size_safe": True, "policy_safe": True}, separators=(",", ":"), sort_keys=True),
            status="pending",
        )
        session.add(change)
        await session.flush()
        session.add(Approval(task_id=task_id, status=ApprovalStatus.PENDING.value, tool_name="prompt.propose_change", approval_type=ApprovalType.CHANGE.value, subject=change.id, request_summary=f"Prompt module change: {module}"))
    ...
    async def apply_change(self, *, session: AsyncSession, change_id: str, user_id: str) -> EvolutionChange:
        change = await self._owned_change(session, change_id, user_id)
        if change.status != "pending":
            raise PromptValidationError("Prompt change is not pending")
        approved = await session.scalar(select(Approval.id).join(Task, Task.id == Approval.task_id).where(Approval.task_id == change.task_id, Approval.approval_type == ApprovalType.CHANGE.value, Approval.subject == change.id, Approval.status == ApprovalStatus.APPROVED.value, Task.user_id == user_id).limit(1))
        if approved is None:
        ...
        session.add(EvolutionVersion(change_id=change.id, user_id=user_id, target_name=change.target_name, previous_checksum=change.base_checksum, new_checksum=change.candidate_checksum, previous_content=current, new_content=change.candidate_content, action="apply"))
        change.status = "applied"
        change.applied_at = utc_now()
        await session.flush()
        return change

    async def rollback(self, *, session: AsyncSession, change_id: str, user_id: str) -> EvolutionChange:
        change = await self._owned_change(session, change_id, user_id)
        if change.status != "applied":
            raise PromptValidationError("Only applied prompt changes can be rolled back")
        applied = await session.scalar(select(EvolutionVersion).where(EvolutionVersion.change_id == change.id, EvolutionVersion.action == "apply").order_by(EvolutionVersion.created_at.desc()).limit(1))
        if applied is None:
        ...
        session.add(EvolutionVersion(change_id=change.id, user_id=user_id, target_name=change.target_name, previous_checksum=applied.new_checksum, new_checksum=applied.previous_checksum, previous_content=current, new_content=applied.previous_content, action="rollback"))
    ...
    def list_versions(self) -> dict[str, object]:
        ...
        if not versions_dir.exists():
            return {"versions": []}
        return {"versions": sorted(path.name for path in versions_dir.glob("*.json"))[:100]}
    ...
    def validate_content(self, content: str) -> str:
        ...
        if _POLICY_DOWNGRADE.search(content):
            raise PromptValidationError("Prompt content attempts to weaken governance policy")
        return content
    ...
    def _write_managed(self, filename: str, content: str) -> None:
        ...
        (versions / f"{filename}.{_checksum(content)[:12]}.json").write_text(json.dumps({"filename": filename, "fingerprint": _checksum(content)}, separators=(",", ":"), sort_keys=True), encoding="utf-8")
    ...
    async def _owned_change(self, session: AsyncSession, change_id: str, user_id: str) -> EvolutionChange:
        change = await session.scalar(select(EvolutionChange).where(EvolutionChange.id == change_id, EvolutionChange.user_id == user_id, EvolutionChange.target_kind == "prompt"))
        if change is None:
...
def _safe_existing_file(path: Path, root: Path, max_bytes: int) -> bool:
    try:
        ...
        return False
    return resolved.is_relative_to(root) and resolved.is_file() and not resolved.is_symlink() and resolved.stat().st_size <= max_bytes
