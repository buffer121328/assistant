...
class ToolDescriptor:
    ...
    input_schema: Mapping[str, Any]  # input_schema 对应的数据字段。
    source_id: str  # source_id 对应的数据字段。
    source_kind: ToolSourceKind  # source_kind 对应的数据字段。
    version: str  # version 对应的数据字段。
    enabled: bool  # enabled 对应的数据字段。
    risk_level: ToolRiskLevel  # risk_level 对应的数据字段。
    requires_approval: bool  # requires_approval 对应的数据字段。
    tags: tuple[str, ...] = ()  # tags 对应的数据字段。
    always_available: bool = False  # always_available 对应的数据字段。
...
class ToolSourceStatus:
    ...
    source_id: str  # source_id 对应的数据字段。
    source_kind: ToolSourceKind  # source_kind 对应的数据字段。
    available: bool  # available 对应的数据字段。
...
class ToolCatalogDiff:
    ...
    added: tuple[str, ...] = ()  # added 对应的数据字段。
    updated: tuple[str, ...] = ()  # updated 对应的数据字段。
    disabled: tuple[str, ...] = ()  # disabled 对应的数据字段。
    removed: tuple[str, ...] = ()  # removed 对应的数据字段。
...
class ToolCatalogSnapshot:
    ...
    revision: int  # revision 对应的数据字段。
    descriptors: tuple[ToolDescriptor, ...]  # descriptors 对应的数据字段。
    sources: tuple[ToolSourceStatus, ...]  # sources 对应的数据字段。
    diff: ToolCatalogDiff = field(default_factory=ToolCatalogDiff)  # diff 对应的数据字段。
...
class ToolRefreshAudit:
    ...
    revision: int  # revision 对应的数据字段。
    sources: tuple[ToolSourceStatus, ...]  # sources 对应的数据字段。
    diff: ToolCatalogDiff  # diff 对应的数据字段。
...
class ToolSelectionResult:
    ...
    reasons: tuple[tuple[str, str], ...]  # reasons 对应的数据字段。
    versions: tuple[tuple[str, str], ...]  # versions 对应的数据字段。
    snapshot_revision: int  # snapshot_revision 对应的数据字段。
