...
from application.enterprise_runtime import knowledge_retrieval_filter
...
def build_knowledge_tool_spec(service: KnowledgeService) -> ToolSpec:
    ...
    async def search(invocation: ToolInvocation) -> list[dict[str, object]]:
        ...
        """
        task = await service.session.get(Task, invocation.task_id)
        profile = (
        ...
        results = await service.search(
            ...
            access_filter=knowledge_retrieval_filter(profile) if profile is not None else None,
        )
