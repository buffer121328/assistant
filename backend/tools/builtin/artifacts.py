...
class OptionalOfficeDependencyError(RuntimeError):
    ...
    def __init__(self) -> None:
        ...
        super().__init__(
            ...
            "Install with: cd backend && uv sync --extra office"
        )
...
class Artifact:
    ...
    reference: str  # reference 对应的数据字段。
...
class ArtifactStore:
    ...
    def reserve_immutable(
        ...
        *,
        ...
        """Reserve an ID-derived target that callers cannot name or overwrite.

        Args:
            task_id: 目标任务 ID。
            artifact_id: 目标产物 ID。
            suffix: 用于执行当前操作的 suffix 参数。
        """
        if not _SAFE_TASK_ID.fullmatch(artifact_id.strip()):
            raise ArtifactPathError("Invalid artifact id")
        target = self.reserve(
            task_id=task_id,
            filename=f"{artifact_id.strip()}{suffix.lower()}",
            suffix=suffix.lower(),
        )
        if target.exists() or target.is_symlink():
            raise ArtifactPathError("Immutable artifact already exists")
        return target

    def atomic_write_immutable(self, target: Path, data: bytes) -> None:
        """Atomically create managed bytes without replacing an existing Artifact.
        ...
        try:
            ...
            raise ArtifactPathError("Immutable artifact already exists") from exc
        finally:
            temporary.unlink(missing_ok=True)
    ...
    def absolute_path(self, *, task_id: str, reference: str) -> Path:
        ...
        if candidate.is_symlink():
            raise ArtifactPathError("Artifact reference must not be a symlink")
        resolved = candidate.resolve(strict=True)
        raw_task_root = self.root / task_id.strip()
        if raw_task_root.is_symlink():
            raise ArtifactPathError("Task artifact directory must not be a symlink")
    ...
    def inspect(self, *, task_id: str, reference: str) -> ArtifactInspection:
        """Hash and size one exact managed file through a bounded streaming read.

        Args:
            task_id: 目标任务 ID。
            reference: 用于执行当前操作的 reference 参数。
        """
        path = self.absolute_path(task_id=task_id, reference=reference)
    ...
    def remove_immutable(self, *, task_id: str, reference: str) -> None:
        """Remove only one exact managed file during failed registration compensation.
