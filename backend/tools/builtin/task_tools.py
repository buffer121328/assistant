...
from application.task_execution.lifecycle import TaskNotFoundError, TaskService
from application.task_execution.events import TaskEventRepository, event_record
...
class AgentTaskToolService:
    """表示 处理 agent task tool service 的后端数据结构或服务对象。"""

    session: AsyncSession  # session 对应的数据字段。
    enqueue_task: Callable[[str], bool] | None = None  # enqueue_task 对应的数据字段。
    max_events: int = 20  # max_events 对应的数据字段。
    max_result_chars: int = 4000  # max_result_chars 对应的数据字段。
    ...
    async def get_result(self, *, user_id: str, task_id: str) -> dict[str, object]:
        ...
        return {
            "task_id": task.id,
            "status": task.status,
            "result_text": _truncate(result or "", self.max_result_chars)
            if result
            else None,
            "error_message": _truncate(task.error_message or "", self.max_result_chars)
    ...
    def _progress(status: str) -> float:
        ...
        return {
            ...
            TaskStatus.RUNNING.value: 0.5,
            TaskStatus.WAITING_APPROVAL.value: 0.5,
            TaskStatus.SUCCESS.value: 1.0,
            TaskStatus.FAILED.value: 1.0,
            TaskStatus.CANCELLED.value: 1.0,
        }.get(status, 0.0)


def build_task_tool_descriptors(*, enabled: bool = True) -> tuple[ToolDescriptor, ...]:
    """构建 task tool descriptors。
...
def build_task_tool_specs(service: AgentTaskToolService) -> tuple[ToolSpec, ...]:
    ...
    async def check(invocation: ToolInvocation) -> Any:
        ...
        return await service.check_status(
            user_id=invocation.user_id,
            task_id=str(invocation.arguments.get("task_id") or ""),
        )
    ...
    async def cancel(invocation: ToolInvocation) -> Any:
        ...
        return await service.cancel(
            user_id=invocation.user_id,
            task_id=str(invocation.arguments.get("task_id") or ""),
            reason=str(invocation.arguments.get("reason") or "cancelled by user"),
        )

    handlers = {
        "task.start_background": start,
        "task.check_status": check,
...
_TASK_TOOL_DEFS: tuple[tuple[str, str, str, dict[str, Any]], ...] = (
    (
        ...
        "L2",
        {
            "type": "object",
            "properties": {
                "conversation_id": _TASK_ID_SCHEMA,
                "task_type": {"type": "string"},
                "input_text": {"type": "string"},
                "workflow_key": {"type": "string"},
                "model_class": {"type": "string"},
            },
            "required": ["conversation_id", "input_text"],
    ...
    (
        ...
        {
            "type": "object",
            "properties": {"task_id": _TASK_ID_SCHEMA, "reason": {"type": "string"}},
            "required": ["task_id"],
            "additionalProperties": False,
        },
    ),
