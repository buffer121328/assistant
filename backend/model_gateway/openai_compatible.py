...
class OpenAICompatibleAdapter:
    ...
    async def chat(self, request: GatewayRequest, node: ModelNode) -> GatewayResult:
        ...
        try:
            ...
            raise self._provider_error(node, f"Invalid provider response: {exc}") from exc
                ...
                async with client.stream(
                    ...
                    async for line in response.aiter_lines():
                        ...
                        try:
                            ...
                            raise self._provider_error(node, "Invalid provider stream") from exc
                        if isinstance(content, str) and content:
                            yield content
        except httpx.TimeoutException as exc:
            raise ModelGatewayError(MODEL_GATEWAY_TIMEOUT, "Model provider timed out", 504) from exc
    ...
    async def _post(
        ...
        async with httpx.AsyncClient(
            ...
            for attempt in range(self.retry_attempts):
                ...
                if response.status_code in {408, 409, 425, 429, 500, 502, 503, 504} and attempt + 1 < self.retry_attempts:
                    continue
    ...
    def _provider_error(self, node: ModelNode, detail: str) -> ModelGatewayError:
        safe = sanitize_text(detail, extra_sensitive_values=(node.api_key, node.base_url))
