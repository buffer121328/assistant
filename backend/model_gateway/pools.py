...
class ModelNode:
    ...
    node_id: str  # node_id 对应的数据字段。
    pool: str  # pool 对应的数据字段。
    provider: str  # provider 对应的数据字段。
    base_url: str  # base_url 对应的数据字段。
    model: str  # model 对应的数据字段。
    api_key: str  # api_key 对应的数据字段。
    capacity: int  # capacity 对应的数据字段。
    cost_advantage: float  # cost_advantage 对应的数据字段。
    enabled: bool = True  # enabled 对应的数据字段。
    latency_target_ms: int = 2000  # latency_target_ms 对应的数据字段。
    rpm_limit: int | None = None  # rpm_limit 对应的数据字段。
    tpm_limit: int | None = None  # tpm_limit 对应的数据字段。
    input_token_cost: float | None = None  # input_token_cost 对应的数据字段。
    output_token_cost: float | None = None  # output_token_cost 对应的数据字段。
...
class NodeMetrics:
    ...
    active_requests: int = 0  # active_requests 对应的数据字段。
    success_count: int = 0  # success_count 对应的数据字段。
    failure_count: int = 0  # failure_count 对应的数据字段。
    ewma_latency_ms: float = 0.0  # ewma_latency_ms 对应的数据字段。
    consecutive_failures: int = 0  # consecutive_failures 对应的数据字段。
    cooldown_until: float | None = None  # cooldown_until 对应的数据字段。
    health_status: str = "healthy"  # health_status 对应的数据字段。
    request_timestamps: tuple[float, ...] = ()  # request_timestamps 对应的数据字段。
    token_timestamps: tuple[
        tuple[float, int], ...
    ] = ()  # token_timestamps 对应的数据字段。
...
def _normalize_pool(value: str) -> str:
    ...
    Args:
        value: value 参数。
    """
    return {"light": "fast", "standard": "reasoning", "rag": "private"}.get(
        value, value
    )
