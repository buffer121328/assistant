...
RAG_TASK_TYPES = frozenset({"rag", "knowledge", "knowledge_answer"})
SUPPORTED_MODEL_CLASSES = frozenset(
    {MODEL_CLASS_LIGHT, MODEL_CLASS_STANDARD, MODEL_CLASS_RAG, "private"}
)
...
class GatewayRequest:
    ...
    model_class: str | None  # model_class 对应的数据字段。
    messages: Sequence[GatewayMessage]  # messages 对应的数据字段。
    temperature: float  # temperature 对应的数据字段。
    max_tokens: int  # max_tokens 对应的数据字段。
...
class GatewayUsage:
    ...
    input_tokens: int  # input_tokens 对应的数据字段。
    output_tokens: int  # output_tokens 对应的数据字段。
...
class GatewayResult:
    ...
    content: str  # content 对应的数据字段。
    usage: GatewayUsage  # usage 对应的数据字段。
    latency_ms: int  # latency_ms 对应的数据字段。
    status: str = "succeeded"  # status 对应的数据字段。
    estimated_cost: float | None = None  # estimated_cost 对应的数据字段。
    diagnostics: dict[str, Any] | None = None  # diagnostics 对应的数据字段。
...
def route_model(task_type: str, model_class: str | None) -> str:
    ...
    if normalized_model_class is not None:
        if normalized_model_class in SUPPORTED_MODEL_CLASSES:
            return (
                MODEL_CLASS_RAG
                if normalized_model_class == "private"
                else normalized_model_class
            )
        raise ModelGatewayError(
    ...
    if normalized_task_type in RAG_TASK_TYPES:
        return MODEL_CLASS_RAG
