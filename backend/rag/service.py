...
class IngestionResult:
    ...
    document_id: str  # document_id 对应的数据字段。
    source_label: str  # source_label 对应的数据字段。
    status: str  # status 对应的数据字段。
    chunk_count: int  # chunk_count 对应的数据字段。
    unchanged: bool  # unchanged 对应的数据字段。
...
class KnowledgeSearchResult:
    ...
    source_label: str  # source_label 对应的数据字段。
    citation: str  # citation 对应的数据字段。
    citation_token: str  # citation_token 对应的数据字段。
    ordinal: int  # ordinal 对应的数据字段。
    content: str  # content 对应的数据字段。
    score: int  # score 对应的数据字段。
    trust_boundary: str  # trust_boundary 对应的数据字段。
    instruction_risk: bool  # instruction_risk 对应的数据字段。
...
class KnowledgeDocumentStatus:
    ...
    status: str  # status 对应的数据字段。
    chunk_count: int  # chunk_count 对应的数据字段。
    last_error_code: str | None  # last_error_code 对应的数据字段。
        ...
        *,
        ...
        if access_filter is None:
            ...
            if "personal" in access_filter.allowed_scopes:
                visibility_terms.append(and_(KnowledgeDocument.visibility == "private", KnowledgeDocument.owner_id == access_filter.owner_id))
            if "department" in access_filter.allowed_scopes and access_filter.organization_ids:
                visibility_terms.append(and_(KnowledgeDocument.visibility == "department", KnowledgeDocument.organization_id.in_(access_filter.organization_ids)))
            if "organization" in access_filter.allowed_scopes and access_filter.organization_ids:
                visibility_terms.append(and_(KnowledgeDocument.visibility == "organization", KnowledgeDocument.organization_id.in_(access_filter.organization_ids)))
            if not visibility_terms:
                return ()
            statement = statement.where(
                KnowledgeChunk.tenant_id == access_filter.tenant_id,
                KnowledgeDocument.tenant_id == access_filter.tenant_id,
                KnowledgeDocument.classification.in_(access_filter.allowed_classifications),
                or_(*visibility_terms),
            )
        rows = await self.session.execute(statement.where(KnowledgeDocument.status == "ready"))
