...
class ConversationCompactionPolicy:
    ...
    trigger_token_threshold: int = 3_000  # trigger_token_threshold 对应的数据字段。
    trigger_message_count: int = 48  # trigger_message_count 对应的数据字段。
    stale_after_tokens: int = 750  # stale_after_tokens 对应的数据字段。
    stale_after_messages: int = 12  # stale_after_messages 对应的数据字段。
    max_source_messages: int = 200  # max_source_messages 对应的数据字段。
...
class ContextSectionTrace:
    ...
    section: SectionName  # section 对应的数据字段。
    estimated_tokens: int  # estimated_tokens 对应的数据字段。
    source_ids: tuple[str, ...] = ()  # source_ids 对应的数据字段。
    truncated_source_ids: tuple[str, ...] = ()  # truncated_source_ids 对应的数据字段。
...
class ContextPack:
    ...
    long_term_memory: str  # long_term_memory 对应的数据字段。
    recent_turns: tuple[ConversationMessageRef, ...]  # recent_turns 对应的数据字段。
    current_input: str  # current_input 对应的数据字段。
    tool_results: tuple[str, ...]  # tool_results 对应的数据字段。
    trace: tuple[ContextSectionTrace, ...]  # trace 对应的数据字段。
    total_estimated_tokens: int  # total_estimated_tokens 对应的数据字段。
    compacted: bool  # compacted 对应的数据字段。
