...
from application.enterprise_runtime import GovernedRetrievalFilter
...
class RetrievalWeights:
    ...
    semantic: float = 0.30  # semantic 对应的数据字段。
    keyword: float = 0.15  # keyword 对应的数据字段。
    importance: float = 0.15  # importance 对应的数据字段。
    recency: float = 0.10  # recency 对应的数据字段。
    confirmation: float = 0.10  # confirmation 对应的数据字段。
    feedback: float = 0.10  # feedback 对应的数据字段。
    scope: float = 0.05  # scope 对应的数据字段。
    link_support: float = 0.05  # link_support 对应的数据字段。
    stale_penalty: float = 0.35  # stale_penalty 对应的数据字段。
    harmful_penalty: float = 0.25  # harmful_penalty 对应的数据字段。
    recency_half_life_days: float = 30.0  # recency_half_life_days 对应的数据字段。
    min_score: float = 0.30  # min_score 对应的数据字段。
    token_budget: int = 500  # token_budget 对应的数据字段。
    max_items: int = 8  # max_items 对应的数据字段。
...
class RetrievedMemory:
    ...
    memory_id: str  # memory_id 对应的数据字段。
    content: str  # content 对应的数据字段。
    memory_type: str  # memory_type 对应的数据字段。
    score: float  # score 对应的数据字段。
    historical: bool  # historical 对应的数据字段。
    injected_tokens: int  # injected_tokens 对应的数据字段。
...
class RetrievalResult:
    ...
    items: tuple[RetrievedMemory, ...]  # items 对应的数据字段。
    trace_id: str  # trace_id 对应的数据字段。
    mode: str  # mode 对应的数据字段。
    time_intent: TimeIntent  # time_intent 对应的数据字段。
    injected_tokens: int  # injected_tokens 对应的数据字段。
...
async def retrieve_memories(
    *,
    ...
    access_filter: GovernedRetrievalFilter | None = None,
) -> RetrievalResult:
    ...
    if semantic_memory is not None and semantic_memory.enabled and query.strip():
        try:
            governed_search = getattr(semantic_memory, "search_governed", None)
            if access_filter is not None and governed_search is not None:
                results = await governed_search(
                    user_id=user_id,
                    query=query.strip(),
                    limit=max(config.max_items * 3, 10),
                    filters=access_filter.provider_filter(),
                )
            else:
                # The legacy provider key is already personal-owner scoped. Shared
                # scopes require explicit governed-filter support and never widen it.
    ...
    memory_query = select(Memory).where(Memory.deleted_at.is_(None))
    if access_filter is None:
        memory_query = memory_query.where(Memory.user_id == user_id)
    else:
        visibility_terms = []
        if "personal" in access_filter.allowed_scopes:
            visibility_terms.append(
                and_(Memory.visibility == "private", Memory.owner_id == access_filter.owner_id)
            )
        if "department" in access_filter.allowed_scopes and access_filter.organization_ids:
            visibility_terms.append(
                and_(Memory.visibility == "department", Memory.organization_id.in_(access_filter.organization_ids))
            )
        if "organization" in access_filter.allowed_scopes and access_filter.organization_ids:
            visibility_terms.append(
                and_(Memory.visibility == "organization", Memory.organization_id.in_(access_filter.organization_ids))
            )
        memory_query = memory_query.where(
            Memory.tenant_id == access_filter.tenant_id,
            or_(*visibility_terms) if visibility_terms else false(),
        )
