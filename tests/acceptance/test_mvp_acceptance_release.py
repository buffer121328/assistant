...
def test_01_docker_compose_release_contracts_are_placeholder_safe() -> None:
    ...
    assert "--extra office" in dockerfile_text
    assert "--extra browser-automation" not in dockerfile_text
...
def test_05_readme_documents_runtime_and_quality_boundaries() -> None:
    ...
    assert "cd backend && uv run ruff check . ../tests" in readme
    assert "cd backend && uv run mypy" in readme
