...
from scripts.smoke.compose_smoke import PROJECT, run_compose_smoke
...
REMOVED_SANDBOX_ENVIRONMENT_KEYS = {
    "SANDBOX_ENABLED",
    "SANDBOX_IMAGE",
...
def test_demo_worker_defaults_to_one_process_and_browser_is_opt_in() -> None:
    compose = yaml.safe_load((ROOT / "docker-compose.yml").read_text(encoding="utf-8"))
    worker = compose["services"]["celery-worker"]
    command = " ".join(str(item) for item in worker["command"])
    environment = worker["environment"]

    assert "--concurrency=${CELERY_WORKER_CONCURRENCY:-1}" in command
    assert environment["CELERY_WORKER_CONCURRENCY"] == "${CELERY_WORKER_CONCURRENCY:-1}"
    assert environment["BROWSER_ENABLED"] == "${BROWSER_ENABLED:-false}"
...
def test_compose_services_include_current_settings_environment_groups() -> None:
    ...
    for service_name in ("assistant-api", "celery-worker"):
        ...
        assert not (REMOVED_SANDBOX_ENVIRONMENT_KEYS & set(environment))
