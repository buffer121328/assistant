...
from domain.models import Base, Conversation, TaskStatus, User
...
def test_agent_model_request_uses_prebudgeted_conversation_history_without_second_slice() -> (
    ...
    from agent.prompting.model_contract import build_agent_model_request
...
def test_local_conversation_token_stats_and_session_continuation(
    ...
    workspace = client.post(
        "/api/spaces",
        json={"user_id": owner.id, "name": "Token Test Workspace"},
    )
    assert workspace.status_code == 201
    workspace_id = workspace.json()["space_id"]
    ...
    assert payload["token_limit"] == 200_000
    assert payload["used_input_tokens"] == 0
    assert payload["used_output_tokens"] == 0
    assert payload["used_total_tokens"] == 0
    assert payload["remaining_tokens"] == 200_000
    assert payload["blocked_reason"] is None
    assert payload["status"] == "ok"

    async def exhaust_conversation() -> None:
        async with sessionmaker() as session:
            conversation = await session.get(Conversation, conversation_id)
            assert conversation is not None
            conversation.used_input_tokens = 200_000
            conversation.token_status = "exhausted"
            conversation.token_blocked_reason = "conversation_token_limit_exceeded"
            await session.commit()

    anyio.run(exhaust_conversation)
    blocked = client.post(
        f"/local/tasks/{second_task['task_id']}/messages",
        json={"user_id": owner.id, "content": "预算耗尽后不能继续"},
    )
    assert blocked.status_code == 409
    assert blocked.json()["error"]["code"] == "conversation_token_limit_exceeded"
