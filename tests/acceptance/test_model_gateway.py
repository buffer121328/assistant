...
def client(sessionmaker: async_sessionmaker[AsyncSession]) -> TestClient:
    settings = Settings(
        ...
        qwen_api_key=SECRET_TOKEN,
        qwen_base_url=PRIVATE_URL,
        qwen_rag_model="qwen-rag-test",
...
@pytest.mark.asyncio
@respx.mock
async def test_05b_rag_task_types_route_to_qwen_private_model(
    client: TestClient,
    sessionmaker: async_sessionmaker[AsyncSession],
) -> None:
    qwen_route = respx.post(f"{PRIVATE_URL}/chat/completions").mock(
        return_value=httpx.Response(200, json=deepseek_success("RAG 千问结果"))
    )
    deepseek_route = respx.post(DEEPSEEK_URL).mock(
        return_value=httpx.Response(200, json=deepseek_success("DeepSeek 结果"))
    )

    user, task = await create_task(sessionmaker, task_type="rag")
    response = client.post(
        "/internal/models/chat",
        json=chat_request(user_id=user.id, task_id=task.id, task_type="rag"),
    )

    assert response.status_code == 200
    assert response.json()["provider"] == "qwen"
    assert response.json()["model"] == "qwen-rag-test"
    assert response.json()["content"] == "RAG 千问结果"
    assert qwen_route.calls.call_count == 1
    assert deepseek_route.calls.call_count == 0
    assert (
        qwen_route.calls.last.request.headers["authorization"]
        == f"Bearer {SECRET_TOKEN}"
    )
    assert '"model":"qwen-rag-test"' in qwen_route.calls.last.request.content.decode()
