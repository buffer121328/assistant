...
from application.task_execution.events import TaskEventPublisher, TaskEventRepository
...
def test_current_environment_docs_match_electron_desktop_boundary() -> None:
    startup_doc = (ROOT / "README.md").read_text(encoding="utf-8")

    for required in (
        "Python 3.12",
        "cd backend && uv sync",
        "cp .env.example .env",
        "cd backend && uv sync --extra browser-automation",
...
def test_desktop_web_release_configuration_excludes_runtime_bloat_and_documents_mode() -> None:
    ...
    smoke_script = ROOT / "scripts" / "smoke" / "desktop_web_release_check.py"
    ...
    assert "out/**" in builder_config["files"]
