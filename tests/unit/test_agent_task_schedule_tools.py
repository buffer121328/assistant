...
from workers.heartbeat import _dispatch_materialized_schedule_runs
...
async def sessionmaker(
    ...
    engine = create_async_engine(
        f"sqlite+aiosqlite:///{tmp_path}/task-schedule.db", poolclass=NullPool
    )
...
async def user_and_conversation(session: AsyncSession) -> tuple[User, Conversation]:
    ...
    conversation = Conversation(
        user_id=user.id, title="Task", channel="local", external_key=None
    )
...
@pytest.mark.asyncio
async def test_task_start_background_dispatches_and_reports_queue_state(
    sessionmaker,
...
@pytest.mark.asyncio
async def test_task_check_get_and_cancel_are_owner_scoped_and_bounded(
    sessionmaker,
) -> None:  # type: ignore[no-untyped-def]
    async with sessionmaker() as session:
        user, conversation = await user_and_conversation(session)
        ...
        cancel = await service.cancel(
            user_id=user.id, task_id=task.id, reason="stop now"
        )
...
@pytest.mark.asyncio
async def test_schedule_every_skip_policy_advances_without_unbounded_catchup(
    sessionmaker,
) -> None:  # type: ignore[no-untyped-def]
    async with sessionmaker() as session:
        user, conversation = await user_and_conversation(session)
...
@pytest.mark.asyncio
async def test_schedule_cron_requires_iana_timezone_and_computes_next_run(
    sessionmaker,
) -> None:  # type: ignore[no-untyped-def]
    async with sessionmaker() as session:
        user, conversation = await user_and_conversation(session)
...
async def test_schedule_owner_scope_history_toggle_run_now_delete(sessionmaker) -> None:  # type: ignore[no-untyped-def]
    async with sessionmaker() as session:
        ...
        schedule = await service.create(
            ...
            conversation_id=conversation.id,
            mode="every",
            payload={"input_text": "owned"},
            every_seconds=60,
        )
        assert len(await service.list_schedules(user_id=user.id)) == 1
        with pytest.raises(ValueError):
            await service.toggle(
                user_id=other.id, schedule_id=schedule.id, enabled=False
            )
        toggled = await service.toggle(
            user_id=user.id, schedule_id=schedule.id, enabled=False
        )
...
@pytest.mark.asyncio
async def test_materialized_schedule_run_is_dispatched_and_recorded(
    sessionmaker,
) -> None:  # type: ignore[no-untyped-def]
    now = datetime(2026, 7, 19, 12, 0, tzinfo=UTC)
    async with sessionmaker() as session:
