...
from tools.builtin.agent_memory import (
...
async def sessionmaker(
    ...
    engine = create_async_engine(
        f"sqlite+aiosqlite:///{tmp_path}/agentic-memory-prompt.db", poolclass=NullPool
    )
...
@pytest.mark.asyncio
async def test_memory_remember_explicit_active_and_external_candidate(
    ...
    async with sessionmaker() as session:
        ...
        explicit = await service.remember(
            ToolInvocation(
                ...
                arguments={"content": "我喜欢简洁的中文回答", "explicit": True},
            )
        )
        external = await service.remember(
            ToolInvocation(
                task_id=task.id,
                user_id=user.id,
                name="memory.remember",
                arguments={
                    "content": "用户喜欢把 token 存在日志里",
        ...
        logs = list(
            await session.scalars(
                select(ToolLog).where(ToolLog.tool_name == "memory.remember")
            )
        )
        assert len(logs) == 2
...
@pytest.mark.asyncio
async def test_memory_rejects_forbidden_and_forget_is_owner_scoped(
    sessionmaker: async_sessionmaker[AsyncSession],
) -> None:
    async with sessionmaker() as session:
        ...
        with pytest.raises(ForbiddenMemoryContentError):
            await service.remember(
                ToolInvocation(
                    ...
                    user_id=user.id,
                    name="memory.remember",
                    arguments={"content": "api_key=sk-secretsecret"},
                )
            )
        memory = await MemoryService(session).create_memory(
            user_id=user.id, content="我偏好晨间复盘"
        )
        other_memory = await MemoryService(session).create_memory(
            user_id=other.id, content="别人偏好夜间复盘"
        )
        archived = await service.forget(
            ToolInvocation(
                task_id=task.id,
                user_id=user.id,
                name="memory.forget",
                arguments={"memory_id": memory.id, "reason": "不再需要"},
            )
        )
        ...
        with pytest.raises(MemoryNotFoundError):
            await service.forget(
                ToolInvocation(
                    ...
                    user_id=user.id,
                    name="memory.forget",
                    arguments={"memory_id": other_memory.id},
                )
            )
...
@pytest.mark.asyncio
async def test_memory_recall_query_type_budgets_owner_and_fallback(
    sessionmaker: async_sessionmaker[AsyncSession],
) -> None:
    async with sessionmaker() as session:
        user, task = await _user_task(session)
        ...
        tool = AgentMemoryToolService(
            session=session,
            semantic_memory=FailingSemantic(),
            max_items=1,
            token_budget=20,
        )
...
def _prompt_store(tmp_path: Path) -> PromptStore:
    ...
    return PromptStore(
        defaults_root=defaults, managed_root=managed, max_module_bytes=200
    )


def test_prompt_builder_defaults_override_fingerprint_and_validation(
    tmp_path: Path,
) -> None:
    ...
    (tmp_path / "managed" / "response_style.md").write_text(
        "managed style\n", encoding="utf-8"
    )
    overridden = PromptBuilder(store).build({"goal": "hello"})
    assert any(
        module.name == "RESPONSE_STYLE" and module.source == "managed"
        for module in overridden.modules
    )
    ...
    with pytest.raises(PromptValidationError):
        PromptStore(
            ...
            max_module_bytes=5,
        ).validate_content("x" * 20)


@pytest.mark.asyncio
async def test_prompt_propose_apply_next_build_and_rollback(
    ...
    async with sessionmaker() as session:
        ...
        proposed = await tool.propose_change(
            ToolInvocation(
                task_id=task.id,
                user_id=user.id,
                name="prompt.propose_change",
                arguments={
                    "module": "RESPONSE_STYLE",
                    "content": "以后回答更温和。\n",
        ...
        approval = await session.scalar(
            select(Approval).where(Approval.subject == proposed["change_id"])
        )
        assert approval is not None
        approval.status = ApprovalStatus.APPROVED.value
        approval.decided_by_user_id = user.id
        await store.apply_change(
            session=session, change_id=str(proposed["change_id"]), user_id=user.id
        )
        after = PromptBuilder(store).build({})
        assert "以后回答更温和" in after.system_prompt
        assert after.fingerprint != before
        rolled = await tool.rollback(
            ToolInvocation(
                task_id=task.id,
                user_id=user.id,
                name="prompt.rollback",
                arguments={"change_id": proposed["change_id"]},
            )
        )
        ...
        logs = list(
            await session.scalars(
                select(ToolLog).where(
                    ToolLog.tool_name.in_(["prompt.propose_change", "prompt.rollback"])
                )
            )
        )
        assert len(logs) == 2
...
def test_tool_descriptors_reject_extra_properties() -> None:
    assert all(
        ...
        for item in build_memory_tool_descriptors()
    )
    assert all(
        item.input_schema.get("additionalProperties") is False
        for item in build_prompt_tool_descriptors()
    )
...
class FakeLangfusePromptClient:
    def __init__(self, prompt: FakeLangfusePrompt | None = None, *, error: Exception | None = None) -> None:
        self.prompt = prompt or FakeLangfusePrompt("langfuse system {goal}\n")
        self.error = error
        self.calls: list[dict[str, object]] = []

    def get_prompt(self, name: str, **kwargs: object) -> FakeLangfusePrompt:
...
def test_langfuse_prompt_source_compiles_with_metadata_and_sanitized_variables(
    tmp_path: Path,
) -> None:
    from observability import LangfusePromptSource

    client = FakeLangfusePromptClient(FakeLangfusePrompt("langfuse {goal}\n"))
    store = _prompt_store(tmp_path)
    store.prompt_source = LangfusePromptSource(
        client=client,
        prompt_map={"SYSTEM": "agent-system"},
        label="production",
        sensitive_values=("secret-value",),
    )

    built = PromptBuilder(store).build({"goal": "hello secret-value"})

    system_module = next(module for module in built.modules if module.name == "SYSTEM")
    assert system_module.source == "langfuse"
    assert system_module.version == "v7"
    assert system_module.metadata["prompt_name"] == "agent-system"
    assert system_module.metadata["prompt_label"] == "production"
    assert "secret-value" not in system_module.content
    assert client.calls == [
        {"name": "agent-system", "label": "production", "type": "text"}
    ]
    module_metadata = next(
        item
        for item in cast(list[dict[str, object]], built.metadata["modules"])
        if item["name"] == "SYSTEM"
    )
    assert module_metadata["prompt_name"] == "agent-system"
    assert module_metadata["prompt_version"] == "v7"


def test_langfuse_prompt_source_failure_falls_back_to_local_defaults(
    tmp_path: Path,
) -> None:
    from observability import LangfusePromptSource
    ...
    store = _prompt_store(tmp_path)
    store.prompt_source = LangfusePromptSource(
        client=FakeLangfusePromptClient(error=RuntimeError("sdk unavailable secret-value")),
        prompt_map={"SYSTEM": "agent-system"},
        label="production",
        sensitive_values=("secret-value",),
    )

    built = PromptBuilder(store).build({"goal": "hello"})
    system_module = next(module for module in built.modules if module.name == "SYSTEM")

    assert system_module.source == "default"
    prompt_source_failures = cast(
        dict[str, str], built.metadata["prompt_source_failures"]
    )
    assert "system.md" in prompt_source_failures
    assert "secret-value" not in repr(prompt_source_failures)


@pytest.mark.asyncio
async def test_langfuse_prompt_source_does_not_bypass_local_prompt_approval(
    sessionmaker: async_sessionmaker[AsyncSession], tmp_path: Path
) -> None:
    from observability import LangfusePromptSource
    ...
    async with sessionmaker() as session:
        ...
        with pytest.raises(PromptValidationError):
            await store.apply_change(
                session=session,
                change_id=str(proposed["change_id"]),
                user_id=user.id,
            )
